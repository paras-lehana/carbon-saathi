...
|---|---|---|---|---|
| 1 | Malformed / malicious request bodies (type confusion, oversized numbers, junk enums) | Every POST body validated by shared zod schemas; failures return `400 VALIDATION_FAILED` with a safe message and no echo of input | [`apps/api/src/middleware/validate.ts`](apps/api/src/middleware/validate.ts), [`packages/core/src/schemas.ts`](packages/core/src/schemas.ts) | implemented |
| 2 | Resource-exhaustion via request flooding | Per-IP token bucket: 60 req/min general, dedicated 10 req/min for the assistant (Gemini calls cost money); `429 RATE_LIMITED` on exceed | [`apps/api/src/middleware/rate-limit.ts`](apps/api/src/middleware/rate-limit.ts) | implemented |
...
| 4 | Cross-origin abuse of the API | CORS allowlist from `ALLOWED_ORIGINS` env (default `http://localhost:3000` only) | [`apps/api/src/server.ts`](apps/api/src/server.ts), [`.env.example`](.env.example) | implemented |
| 5 | Header/content-type attacks, MIME sniffing, clickjacking | helmet with `default-src 'none'` CSP for the API surface (it serves JSON, nothing executable), `x-powered-by` disabled | [`apps/api/src/server.ts`](apps/api/src/server.ts) | implemented |
...
| 8 | Secret management at production scale | Roadmap: Google Secret Manager + least-privilege service account on Cloud Run; the central `config.ts` loader makes this a one-file swap | [`apps/api/src/config.ts`](apps/api/src/config.ts), [`tasks.md`](tasks.md) Phase 11 | planned |
...
| 10 | Trusting client-supplied `userId` (impersonation) | Acceptable in the keyless demo tier (no PII at stake, in-memory state); honest limitation. Roadmap replaces it with Firebase Auth ID-token verification (Bearer) and Firestore owner-only rules | [`tasks.md`](tasks.md) Phase 7 | planned |
| 11 | Gaming the points system (inflated action logs) | Per-action `maxPerDay` caps enforced in the deterministic core, not the client; unknown ids and non-positive quantities rejected | [`packages/core/src/actions.ts`](packages/core/src/actions.ts) | implemented |
