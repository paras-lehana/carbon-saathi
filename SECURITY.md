...
| 5   | Header/content-type attacks, XSS, MIME sniffing, clickjacking                                                 | API: helmet with `default-src 'none'` CSP (it serves JSON, nothing executable), `x-powered-by` disabled. Web origin: full `Content-Security-Policy` (self-only sources, `object-src 'none'`, `frame-ancestors 'none'`), `Permissions-Policy` denying camera/mic/geolocation/payment, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, HSTS with preload, `poweredByHeader` off | [`apps/api/src/server.ts`](apps/api/src/server.ts), [`apps/web/next.config.ts`](apps/web/next.config.ts)                                                                                                                    | implemented |
...
This is a hackathon project; there is no bug-bounty programme. Please report
vulnerabilities **privately** — public issues would disclose an unpatched flaw to
everyone watching the repository:

- Email: [paras.lehana@indiamart.com](mailto:paras.lehana@indiamart.com)
- GitHub [private vulnerability reporting](https://github.com/paras-lehana/carbon-saathi/security/advisories/new)
- Machine-readable contact: [`/.well-known/security.txt`](apps/web/public/.well-known/security.txt)
  is served by the live web origin per RFC 9116.

Expect acknowledgement within 7 days. Coordinated disclosure window: 90 days from
report, sooner once a fix ships. Non-sensitive bugs (UI, copy, behaviour) are welcome
